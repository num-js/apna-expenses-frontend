...
import { fetchAllMoneyData, deleteMoney } from '../../store/reducers/expensesReducer';
...
const AccountPage = () => {
    const { allMoneyData } = useSelector(state => state.expensesReducer);
    const dispatch = useDispatch();
    const [toggleBottomSheet, setToggleBottomSheet] = useState(false);

    useEffect(() => {
        !allMoneyData && dispatch(fetchAllMoneyData());
    }, []);
                    ...
                    <section class="text-gray-600 body-font" style={{ minHeight: '90vh' }}>
                        <div class="container px-5 py-4 mx-auto">
                            <div class="flex flex-wrap -m-2">
                                {allMoneyData && allMoneyData.map((transaction) => (
                                    <div key={transaction._id} class="p-2 lg:w-1/3 md:w-1/2 w-full">
                                        <div class="h-full flex items-center justify-between border-gray-200 border p-4 rounded-lg">
                                            <div class="">
                                                <h2 class="text-white title-font font-medium"> {transaction.title} </h2>
